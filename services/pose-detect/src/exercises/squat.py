...
class Squat(KeyInterestPoint):
  ...
  def calculate_inside_knee_angle(self, key_points: Tuple, idx_to_coordinates: Dict[int, tuple[int, int]]) -> KeyInterestPoint2D:
    ...
    rotation_angle = self.calculate_angle((knee_x + 90, knee_y), knee_coord, ankle_coord)
  ...
  def calculate_outside_hip_angle(self, key_points: Tuple, idx_to_coordinates) -> KeyInterestPoint2D:
    ...
    idx_to_result = {
      shoulder: shoulder_coord,
      hip: hip_coord,
      knee: knee_coord
    }
    
    angle = self.calculate_angle(shoulder_coord, hip_coord, knee_coord)
    rotation_angle = self.calculate_angle((hip_x + 90, hip_y), hip_coord, knee_coord)
  ...
  def get_2d_key_points(self, result, camera_view: CameraView) -> Dict[str, KeyInterestPoint2D]:
    idx_to_coordinates = {idx: (landmark.x, landmark.y) for idx, landmark in enumerate(result)}

    key_points_2d = {}
    for name, key_points in self.CAMERA_VIEW_TO_KEY_POINT_INDEX[camera_view].items():
      match name:
        case "OUTSIDE_HIP":
          key_points_2d[name] = self.calculate_outside_hip_angle(key_points, idx_to_coordinates)
        case "INSIDE_KNEE":
          key_points_2d[name] = self.calculate_inside_knee_angle(key_points, idx_to_coordinates)
        case _:
